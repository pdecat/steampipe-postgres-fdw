...
//    elog(WARNING, "Caught signal %d", sig);
}
...
static bool fdwIsForeignScanParallelSafe(PlannerInfo *root, RelOptInfo *rel, RangeTblEntry *rte) {
  // Use WARNING level instead of DEBUG1 to ensure visibility
  elog(WARNING, "fdwIsForeignScanParallelSafe called - checking environment variable");

  const char *parallel_safe_env = getenv("STEAMPIPE_FDW_PARALLEL_SAFE");
  bool parallel_safe = parallel_safe_env != NULL;

  if (parallel_safe) {
    elog(WARNING, "STEAMPIPE_FDW_PARALLEL_SAFE is set: %s", parallel_safe_env);
  } else {
    elog(WARNING, "STEAMPIPE_FDW_PARALLEL_SAFE is NOT set - parallel execution will be disabled");
  }

  elog(WARNING, "fdwIsForeignScanParallelSafe returning: %s", parallel_safe ? "true" : "false");
...
static void fdwGetForeignRelSize(PlannerInfo *root, RelOptInfo *baserel, Oid foreigntableid)
{
  ...
  elog(WARNING, "fdwGetForeignRelSize called - checking if parallel safety callback will be invoked");
...
static void fdwGetForeignPaths(PlannerInfo *root, RelOptInfo *baserel, Oid foreigntableid)
{
  ...
  elog(WARNING, "fdwGetForeignPaths called - parallel execution may be evaluated here");
