...
//	if !i.CanIterate() {
//		// this is a bug
//		log.Printf("[WARN] scanIterator cannot iterate: connection %s, status: %s", i.GetConnectionName(), i.Status())
//		return nil, fmt.Errorf("scanIterator cannot iterate: connection %s, status: %s", i.GetConnectionName(), i.Status())
//	}
//
//	row := <-i.rows
//
