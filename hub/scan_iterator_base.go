...
func (i *scanIteratorBase) Next() (map[string]interface{}, error) {
	...
	FdwLogMessage(1, fmt.Sprintf("[DEBUG] Worker PID %d: About to read from rows channel - iterator %p, channel len: %d", os.Getpid(), i, len(i.rows)))
	row := <-i.rows
	FdwLogMessage(1, fmt.Sprintf("[DEBUG] Worker PID %d: Read from rows channel completed - iterator %p, got row: %v", os.Getpid(), i, row != nil))
...
func (i *scanIteratorBase) readThread(ctx context.Context) {
	FdwLogMessage(1, fmt.Sprintf("[DEBUG] Worker PID %d: readThread starting - iterator %p", os.Getpid(), i))
	...
	if i.status == QueryStatusStarted {
		...
		FdwLogMessage(1, fmt.Sprintf("[DEBUG] Worker PID %d: readThread finished readPluginResult loop - iterator %p", os.Getpid(), i))
	} else {
		FdwLogMessage(1, fmt.Sprintf("[DEBUG] Worker PID %d: readThread skipping - iterator %p status is %s (not STARTED)", os.Getpid(), i, i.status))
	}

	// now we are done
	FdwLogMessage(1, fmt.Sprintf("[DEBUG] Worker PID %d: readThread about to close rows channel - iterator %p", os.Getpid(), i))
	close(i.rows)
}
...
func (i *scanIteratorBase) readPluginResult(ctx context.Context) bool {
	...
	go func() {
		...
		FdwLogMessage(1, fmt.Sprintf("[DEBUG] Worker PID %d: About to call pluginRowStream.Recv() - iterator %p", os.Getpid(), i))
		rowResult, err := i.pluginRowStream.Recv()
		FdwLogMessage(1, fmt.Sprintf("[DEBUG] Worker PID %d: pluginRowStream.Recv() returned - iterator %p, result: %v, err: %v", os.Getpid(), i, rowResult != nil, err))
	...
	FdwLogMessage(1, fmt.Sprintf("[DEBUG] Worker PID %d: readPluginResult entering select statement - iterator %p", os.Getpid(), i))
	select {
